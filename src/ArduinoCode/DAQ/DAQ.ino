...
void loop(){
  if (rfidModule.check() == true) //Check to see if any new data has come in from module
  {
    ...
    else if (responseType == RESPONSE_IS_TAGFOUND)
    {
      ...
      dataFile.print(TAG.rssi);
      Serial.print(F("]"));
      dataFile.print(F("]"));

      Serial.print(F(" freq["));
      dataFile.print(F("freq["));
      Serial.print(TAG.freq);
      dataFile.print(TAG.freq);
      Serial.print(F("]"));
      dataFile.print(F("]"));

      Serial.print(F(" time["));
      dataFile.print(F(" time["));
      Serial.print(TAG.timeStamp);
      dataFile.print(TAG.timeStamp);
      Serial.print(F("]"));
      dataFile.print(F("]"));

      //Print EPC bytes, this is a subsection of bytes from the response/msg array
      Serial.print(F(" epc["));
      dataFile.print(F(" epc"));
      for (byte x = 0 ; x < TAG.tagEPC ; x++)
      {
